...
export const StringDiagram: React.FC<StringDiagramProps> = ({ sites = [], loading = false }) => {
    ...
    const displayedLoggers = React.useMemo(() => {
        ...
        // Find the NAME of the selected logger ID
        const selectedLoggerName = availableLoggers.find(l => l.id === selectedLoggerId)?.name;

        if (!selectedLoggerName) return [];

        // Return ALL loggers that match that name (aggregating data from multiple sites/instances)
        return availableLoggers.filter(l => l.name === selectedLoggerName);
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                        ...
                        >
                            ...
                            {/* Deduplicate loggers for dropdown only */}
                            {Array.from(new Set(availableLoggers.map(l => l.name)))
                                .map(name => {
                                    const logger = availableLoggers.find(l => l.name === name);
                                    if (!logger) return null;
                                    // Calculate total inverters for this logger name across all instances
                                    const totalInverters = availableLoggers
                                        .filter(l => l.name === name)
                                        .reduce((acc, curr) => acc + (curr.inverters?.length || 0), 0);

                                    return (
                                        <option key={logger.id} value={logger.id}>
                                            {logger.name} ({totalInverters} Inv)
                                        </option>
                                    );
                                })
                            }
