...
const WeatherIcon = ({ condition, className }: { condition: string, className?: string }) => {
    switch (condition) {
        ...
        case 'clear': return <Moon className={cn("text-blue-200", className)} />;
        case 'cloudy-sun': return <CloudSun className={cn("text-yellow-200", className)} />;
        case 'cloudy-moon': return <CloudMoon className={cn("text-slate-400", className)} />;
    ...
    return (
        <div className={cn("flex items-center gap-3 px-3 py-1.5 bg-slate-800/40 rounded-full border border-slate-700/50 hover:bg-slate-800/60 transition-colors", className)}>
            ...
            <div className="flex flex-col leading-none">
                <span className="text-sm font-bold text-white tabular-nums">{weather.temperature}°C</span>
