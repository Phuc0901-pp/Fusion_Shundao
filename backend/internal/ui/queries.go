...
func fetchProductionDataFromVM() []ProductionDataPoint {
	...
	// Start of TODAY (00:00) using utils.Now() location
	start := time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, utils.GetLocation())
	end := now
...
func fetchMonthlyProductionData() []MonthlyDataPoint {
	...
	startDate := time.Date(2026, 2, 9, 0, 0, 0, 0, utils.GetLocation())
	endDate := time.Date(now.Year(), now.Month(), now.Day(), 23, 59, 59, 0, utils.GetLocation())
